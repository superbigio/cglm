...
/*
 Functions:
   ...
   CGLM_INLINE vec3s glms_vec3_recip(vec3s v);
   CGLM_INLINE vec3s glms_vec3_rsqrt(vec3s v);
 */
...
CGLM_INLINE
vec3s
glms_vec3_recip(vec3s v) {
  vec3s r;
  glm_vec3_recip(v.raw, r.raw);
  return r;
}
...
CGLM_INLINE
vec3s
glms_vec3_rsqrt(vec3s v) {
  vec3s r;
  glm_vec3_rsqrt(v.raw, r.raw);
  return r;
}
